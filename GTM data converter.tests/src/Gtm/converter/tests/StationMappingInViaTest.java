...
public class StationMappingInViaTest {
	...
	public void setUp() throws Exception {
		...
		LegacyDataFactory.addStation(tool,"1","00001",c);
	...
	@Test 
	public void testStationMappingInVia() {
		...
		for (RegionalConstraint r : tool.getGeneralTariffModel().getFareStructure().getRegionalConstraints().getRegionalConstraints()) {
			...
			if (!TestUtils.isReturnRoute(r)) {
				...
				assert(route.getRoute().getStations().get(0).getStation().getCode() == "00001");
				...
				assert(route.getRoute().getStations().get(6).getStation().getCode() == "00001");
