	...
	public void testStationMappingReversed() {
		...
		for (RegionalConstraint r : tool.getGeneralTariffModel().getFareStructure().getRegionalConstraints().getRegionalConstraints()) {
			...
			if (!TestUtils.isReturnRoute(r)) {
				...
				assert(r.getExitConnectionPoint().getConnectedStationSets().get(0).getStations().get(0).getCode().equals("10000"));

				
				ViaStation route = r.getRegionalValidity().get(0).getViaStation();
				
				assert(route.getRoute().getStations().get(0).getStation().getCode() == "00007");
				assert(route.getRoute().getStations().get(6).getStation().getCode() == "10000");
				
				assert (route.getRoute().getStations().size() == 7);
				
			} else {
				
				assert(r.getEntryConnectionPoint().getConnectedStationSets().get(0).getStations().get(0).getCode().equals("10000"));
				assert(r.getExitConnectionPoint().getConnectedStationSets().get(0).getStations().get(0).getCode().equals("00007"));
			
				ViaStation route = r.getRegionalValidity().get(0).getViaStation();

				assert(route.getRoute().getStations().get(0).getStation().getCode() == "10000");
				assert(route.getRoute().getStations().get(6).getStation().getCode() == "00007");
