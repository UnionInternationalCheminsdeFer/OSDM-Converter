...
public class WaldshutTest {
	...
	public void setUp() throws Exception {
		...
		LegacyDataFactory.addStation(tool,"W","20000",TestUtils.findCountry(tool,98));
		...
		side1.getStations().getStations().add(TestUtils.findStation(tool, 98,"10000")); //Z-Town
		...
		side2.getStations().getStations().add(TestUtils.findStation(tool, 98,"10000")); //Z-Town
		side2.getStations().getStations().add(TestUtils.findStation(tool, 98,"20000")); //W-Town	
	...
	public void testVirtualNonMeritsBorderStationConversion() {
		...
		for (ConnectionPoint p : tool.getGeneralTariffModel().getFareStructure().getConnectionPoints().getConnectionPoints()) {
			...
			if (p.getLegacyBorderPointCode() == borderPointCode) {
				borderFound = true;
			}
		}
		assert(borderFound);
		...
		for (RegionalConstraint r : tool.getGeneralTariffModel().getFareStructure().getRegionalConstraints().getRegionalConstraints()) {
			...
			if (isReturnRoute) {
				assert(description.equals("G*F*E*D*C*B*A (GR)"));
			} else {
				assert(description.equals("A (GR)*B*C*D*E*F*G"));
			}
		}
