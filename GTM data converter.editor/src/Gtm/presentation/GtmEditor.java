...
public class GtmEditor
	 ...
	 * @generated
	 */
	public static final List<String> FILE_EXTENSION_FILTERS = prefixExtensions(GtmModelWizard.FILE_EXTENSIONS, "*.");
			...
			public void partActivated(IWorkbenchPart p) {
				if (p instanceof ContentOutline) {
					if (((ContentOutline)p).getCurrentPage() == contentOutlinePage) {
						getActionBarContributor().setActiveEditor(GtmEditor.this);
	...
	protected void handleActivate() {
		...
		if (!removedResources.isEmpty()) {
			if (handleDirtyConflict()) {
				getSite().getPage().closeEditor(GtmEditor.this, false);
			}
			else {
	...
	protected void initializeEditingDomain() {
		...
		adapterFactory.addAdapterFactory(new GtmItemProviderAdapterFactory());
	...
	public void doSaveAs() {
		...
		String[] files = GtmEditorAdvisor.openFilePathDialog(getSite().getShell(), SWT.SAVE, filters);
		if (files.length > 0) {
	...
	private static String getString(String key) {
		return GtmEditorPlugin.INSTANCE.getString(key);
	}
	...
	private static String getString(String key, Object s1) {
		return GtmEditorPlugin.INSTANCE.getString(key, new Object [] { s1 });
	}
