...
import gtmV30.LuggageDimension;
import gtmV30.LuggageDimension.Dimension;
...
import gtmV30.TrainValidity.Scope;
...
public class GTMJsonImporterV30 {
	...
	public GTMJsonImporterV30(GTMTool tool, EditingDomain domain, GtmEditor editor) {
		this.tool = tool;
		this.editor = editor;
	...
	private Gtm.LuggageConstraint convert(LuggageConstraint l) {
		...
		if (l.getRestrictedLuggageItems() != null && !l.getRestrictedLuggageItems().isEmpty()) {
			for (gtmV30.LuggageRestriction ri : l.getRestrictedLuggageItems()) {
	...
	private FareConstraintBundles convertFareConstraintBundles(List<gtmV30.FareConstraintBundle> fareConstraintBundles) {
		
		FareConstraintBundles bs = GtmFactory.eINSTANCE.createFareConstraintBundles();
		if (fareConstraintBundles == null) return bs;
		for (gtmV30.FareConstraintBundle bJ : fareConstraintBundles) {
	...
	private TrainValidity convert(gtmV30.TrainValidity trainValidity) {
	...
	private ValidityRange convert(gtmV30.ValidityRange jv) {
	...
	private Collection<? extends SalesRestriction> convertSalesRestrictions(List<gtmV30.SalesRestriction> jl) {

		ArrayList<SalesRestriction> l = new ArrayList<SalesRestriction>();
		if (jl == null || jl.isEmpty()) return l;
		
		for (gtmV30.SalesRestriction jr : jl) {
	...
	private ReservationOptions convert(gtmV30.ReservationOptions jr) {
		if (jr == null) return null;
	...
	private Collection<? extends PassengerCombinationConstraint> convertPassengerCombinationList(List<gtmV30.CombinationConstraint> jl) {
		ArrayList<PassengerCombinationConstraint> l = new ArrayList<PassengerCombinationConstraint>();
		if (jl == null || jl.isEmpty()) return l;
		for (gtmV30.CombinationConstraint jc : jl) {
	...
	private FareConstraintBundle convert(gtmV30.FareConstraintBundle jf) {
	...
	private Collection<? extends RegulatoryCondition> convert(List<String> regulatoryConditions) {
